    ...
    <main className="min-h-screen flex" style={{ fontFamily: 'var(--fonte-principal)' }}>
      <div className="w-full lg:w-1/2 xl:w-2/5 flex flex-col justify-center px-6 py-12 lg:px-12 bg-white">
        <div className="text-center mb-8">
          ...
          />
          <h1 className="text-3xl md:text-4xl font-bold mb-4" style={{ color: 'var(--cinza-escuro)' }}>
            Criar sua conta
          </h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-md mx-auto w-full">
          ...
          <div className="space-y-2">
            ...
            <input
              id="nome"
              type="text"
              className={`w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-(--roxo-vibrante) focus:border-transparent transition-all duration-200 bg-white ${errors.nome ? 'border-red-500 focus:ring-red-500' : ''
                }`}
            ...
            />
            {errors.nome && <span className="text-red-600 text-sm font-medium block mt-1">{errors.nome.message}</span>}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                {...register('senha', {
                  ...
                  pattern: { value: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/, message: 'Senha deve conter letras maiúsculas, minúsculas e números' },
                })}
              />
              <button
                type="button"
            ...
            {password && !errors.senha && (
              <div className="flex items-center gap-3 mt-2">
                <div
                  className={`h-1 flex-1 rounded-full bg-gray-200 transition-all duration-300 ${password.length >= 6 ? 'bg-green-500 w-full' : 'bg-red-500 w-1/4'
                    }`}
                ></div>
                <span className="text-xs text-gray-500 font-medium">{password.length >= 6 ? 'Senha forte' : 'Senha fraca'}</span>
              </div>
            )}
          </div>
          <div className="space-y-2">
            ...
            >
              ...
              <option value="RH_CLEVEL">RH - C-Level</option>
